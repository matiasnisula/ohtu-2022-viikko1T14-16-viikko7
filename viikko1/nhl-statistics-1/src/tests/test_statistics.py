...
class TestStatistics(unittest.TestCase):
    ...
    def test_search_returns_player_if_exits(self):
        self.player = self.statistics.search("Kurri")

        self.assertEqual(self.player.name, "Kurri")

    def test_search_returns_none_if_player_doesnt_exits(self):
        self.player = self.statistics.search("Litmanen")

        self.assertEqual(self.player, None)

    def test_team_returns_correct_players(self):
        self.team = self.statistics.team("EDM")
    ...
    def test_top_scorers_returns_players_in_correct_order(self):
        self.top_5 = self.statistics.top_scorers(5)
        correct_top_5 = ["Gretzky", "Lemieux", "Yzerman", "Kurri", "Semenko"]
        correct_order = True

        for i in range(len(self.top_5)):
            if correct_top_5[i] != self.top_5[i].name:
                correct_order = False
